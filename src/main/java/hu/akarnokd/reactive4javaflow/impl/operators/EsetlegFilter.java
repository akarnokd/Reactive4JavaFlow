...
public final class EsetlegFilter<T> extends Esetleg<T> {
    ...
    protected void subscribeActual(FolyamSubscriber<? super T> s) {
        if (s instanceof ConditionalSubscriber) {
            source.subscribe(new FolyamFilter.FilterConditionalSubscriber<>((ConditionalSubscriber<? super T>) s, predicate));
        } else {
            source.subscribe(new FolyamFilter.FilterSubscriber<>(s, predicate));
        }
    }
}
