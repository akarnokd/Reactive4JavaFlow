 ...
 */
public final class ParallelJoin<T> extends Folyam<T> {
    ...
    public ParallelJoin(ParallelFolyam<? extends T> source, int prefetch, boolean delayErrors) {
        this.source = source;
    ...
    implements Flow.Subscription {
        ...
        JoinSubscriptionBase(FolyamSubscriber<? super T> actual, int n, int prefetch) {
            this.actual = actual;
    ...
    static final class JoinSubscription<T> extends JoinSubscriptionBase<T> {
        ...
        JoinSubscription(FolyamSubscriber<? super T> actual, int n, int prefetch) {
        ...
        public void onNext(JoinInnerSubscriber<T> inner, T value) {
            if (get() == 0 && compareAndSet(0, 1)) {
                if (requested.get() != 0) {
                    ...
                    if (!q.offer(value)) {
                        cancelAll();
                        Throwable mbe = new IllegalStateException("Queue full?!");
                        if (errors.compareAndSet(null, mbe)) {
                            actual.onError(mbe);
                        } else {
                            FolyamPlugins.onError(mbe);
                        }
                        return;
                    }
                }
    ...
    static final class JoinSubscriptionDelayError<T> extends JoinSubscriptionBase<T> {
        ...
        JoinSubscriptionDelayError(FolyamSubscriber<? super T> actual, int n, int prefetch) {
            super(actual, n, prefetch);
        }
