...
public final class FolyamSumFloat<T> extends Esetleg<Float> {
    ...
    public FolyamSumFloat(Folyam<T> source, CheckedFunction<? super T, ? extends Number> valueSelector) {
        this.source = source;
    ...
    static final class SumIntSubscriber<T> extends DeferredScalarSubscription<Float> implements FolyamSubscriber<T> {
        ...
        SumIntSubscriber(FolyamSubscriber<? super Float> actual, CheckedFunction<? super T, ? extends Number> valueSelector) {
            super(actual);
            this.valueSelector = valueSelector;
        }
        ...
        public void onNext(T item) {
            if (getPlain() != CANCELLED) {
                ...
                try {
                    sum += Objects.requireNonNull(valueSelector.apply(item), "The valueSelector returned a null Number").floatValue();
                } catch (Throwable ex) {
                    upstream.cancel();
                    error(ex);
                }
