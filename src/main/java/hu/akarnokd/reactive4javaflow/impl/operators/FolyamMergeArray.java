...
public final class FolyamMergeArray<T> extends Folyam<T> {
    ...
    public FolyamMergeArray(Flow.Publisher<? extends T>[] sources, boolean delayError) {
    ...
    protected void subscribeActual(FolyamSubscriber<? super T> s) {
        ...
        FolyamSubscriber<? super Flow.Publisher<? extends T>> subscriber =
                FolyamFlatMap.createSubscriber(s, v -> v, Integer.MAX_VALUE, FolyamPlugins.defaultBufferSize(), delayError);
    ...
    public static <T> Folyam<T> mergeWith(Folyam<T> source, Flow.Publisher<? extends T> other, boolean delayError) {
        if (source instanceof FolyamMergeArray) {
            FolyamMergeArray aa = (FolyamMergeArray) source;
            if (aa.delayError == delayError) {
                int n = aa.sources.length;
                Flow.Publisher<? extends T>[] newSrcs = Arrays.copyOf(aa.sources, n + 1);
                newSrcs[n] = other;
                return new FolyamMergeArray<>(newSrcs, delayError);
            }
        }
        return new FolyamMergeArray<>(new Flow.Publisher[] { source, other }, delayError);
