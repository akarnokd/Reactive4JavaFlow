...
public final class EsetlegReplay<T, R> extends Esetleg<R> {
    ...
    public EsetlegReplay(Esetleg<T> source, CheckedFunction<? super Folyam<T>, ? extends Esetleg<? extends R>> handler) {
        this.source = source;
    ...
    protected void subscribeActual(FolyamSubscriber<? super R> s) {
        CachingProcessor<T> mp = CachingProcessor.withCapacityHint(1);
