...
public class IOSchedulerServiceStandardTest extends AbstractSchedulerServiceTest {

    @Override
    protected SchedulerService create() {
        return SchedulerServices.io();
    }

    @Test
    public void shutdownDirectRejects() {
        TestHelper.withErrorTracking(errors -> {
            SchedulerService sch = SchedulerServices.newIO("A");
            sch.shutdown();
    ...
    public void zeroPeriod() throws InterruptedException {
        ...
        d.replace(SchedulerServices.io().schedulePeriodically(() -> {
            cdl.countDown();
            if (cdl.getCount() == 0) {
                d.close();
            }
