...
public class FolyamTimeoutTimedFallbackTest {

    @Test
    public void standard() {
        TestHelper.assertResult(
                Folyam.range(1, 5)
                .timeout(1, TimeUnit.MINUTES, SchedulerServices.single(), Folyam.just(6))
                , 1, 2, 3, 4, 5
        );
    }
    ...
    public void error() {
        TestHelper.assertFailureComposed(-1,
                f -> f.timeout(1, TimeUnit.MINUTES, SchedulerServices.single(), Folyam.just(6)),
                IOException.class);
    }
    ...
    public void timeoutFirst() {
        Folyam.never()
                .timeout(10, TimeUnit.MILLISECONDS, SchedulerServices.single(), Folyam.just(6))
                .test()
    ...
    public void fallbackError() {
        Folyam.never()
                .timeout(10, TimeUnit.MILLISECONDS, SchedulerServices.single(), Folyam.error(new IOException()))
                .test()
                .awaitDone(5, TimeUnit.SECONDS)
    ...
    public void fallbackBackpressured() {
        Folyam.range(1, 5).concatWith(Folyam.never())
                .timeout(10, TimeUnit.MILLISECONDS, SchedulerServices.single(), Folyam.range(6, 5))
            .test(6)
